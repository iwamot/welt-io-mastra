...
- `create_sample_file` — writes a small CSV and returns it as a media part, which the model reads and Welt uploads to the thread as `sample.csv`. Mastra's `requireToolApproval` holds the call before it runs, so the thread gets **Approve** / **Reject** buttons first — the framework asking, where `sample_dangerous_action` below asks from inside the tool. Ask it for a sample file, then approve.
